...
class model
{
    ...
    function __construct()
    {
        ...
        $this->conn = new mysqli('localhost', 'root', '', 'swayamtech');
    }
    ...
    function double_select($tbl1, $tbl2, $col, $on)
    {
        ...
        // SELECT with WHERE condition(s)
    function select_where($tbl, $arr)
    {
        $sel = "SELECT * FROM $tbl WHERE 1=1";
        foreach ($arr as $col => $val) {
            $sel .= " AND $col = '" . $this->conn->real_escape_string($val) . "'";
        }
        $run = $this->conn->query($sel);
        return $run; // mysqli_result object
    }
